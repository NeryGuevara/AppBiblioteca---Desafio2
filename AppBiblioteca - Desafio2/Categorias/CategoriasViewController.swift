...
class CategoriasViewController: UIViewController {
    ...
    var dataSource = MenuCategorias(titulo: "", categorias: [])
    ...
    func getData(){
        ...
        let horror = Categoria(nombre: "Horror", libros: [it, apocalipsis])
        
        let ficcion = Categoria(nombre: "Ficción", libros: [lazosDeAmor, laOdisea, elClub, elInstituto])
        
        let autoayuda = Categoria(nombre: "Autoayuda", libros: [muchasVidas, muerte])
        
        let suspenso = Categoria(nombre: "Suspenso", libros: [laHistoria, laChica])
        
        let datos = MenuCategorias(titulo: "Categorías", categorias: [horror, ficcion, autoayuda, suspenso])
