...
class AutoresViewController: UIViewController {
    ...
    func getData(){
        ...
        let johnKat = Autor(nombre: "John Katzenbach", libros: [elClub, laHistoria])
        
        let paulaHawkins = Autor(nombre: "Paula Hawkins", libros: [laChica])
        
        let stephenKing = Autor(nombre: "Stephen King", libros: [it, elInstituto, apocalipsis])
        
        let autores = MenuAutores(titulo: "Autores", autores: [brianWeiss, federicoOrtiz, homero,johnKat,paulaHawkins,stephenKing])
        dataSource = autores
...
extension AutoresViewController : UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        label.text = dataSource.autores?[section].nombre ?? ""
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        return dataSource.autores?.count ?? 0
    }
    
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return dataSource.autores?[section].libros?.count ?? 0
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        
        let libro = dataSource.autores?[indexPath.section].libros?[indexPath.row]
